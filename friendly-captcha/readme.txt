...
* New installations now use Friendly Captcha v2 by default. Existing installations keep their current setting and can switch under Settings > Friendly Captcha.
...
* Only show the admin verification-failed notice for configuration errors
* The notice now shows the time of the last failure and auto-dismisses a week after the last failure
...
* Add support for WP Job Openings
...
* Reset widget on error in WooCommerce checkout

= 1.15.21 =

* Fix bug causing API keys to be overwritten with invalid values.
